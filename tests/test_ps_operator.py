...
IMAGE_TAG = "0.8.0" #FIXME: Get this from cr version
...
def helm_resources(helm_template):
    resources = helm_template(
        chart_path=CHART_PATH, release_name=RELEASE_NAME, values=VALUES_PATH
    )
    return resources


@pytest.fixture(scope="class")
def bundle_resources(get_resources_from_bundle):
    resources = get_resources_from_bundle(OPERATOR_PROJECT_NAME, IMAGE_TAG)[:-1] #FIXME: The last item is none
    return resources
...
class TestHelmDefaultValuesAgainstBundle:
